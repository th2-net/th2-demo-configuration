...
def aggressive_ioc_traded_against_two_orders_partially_and_then_cancelled(case_name, report_id, input_parameters,
    ...
    # ######SENDING ORDER1############################################
    # General parameters for several orders
    ...
    order1_response = sf.placeOrderFIX(
        ...
        place_message_request=PlaceMessageRequest(
            description=f'Trader "{input_parameters["trader1"]}" sends request to create Order.',
    ...
    # ######REQUEST TO VERIFY EXECUTION REPORT ON ORDER1##############
    # FIX header
    basic_header = {
    ...
    order2_response = sf.placeOrderFIX(
        act=factory['act'],
        place_message_request=PlaceMessageRequest(
            description=f'Trader "{input_parameters["trader1"]}" '
            f'sends request to create Order with price lower than first order.',
            connection_id=ConnectionID(session_alias=input_parameters['trader1_fix']),
            parent_event_id=input_parameters['case_id'],
            message=sf.create_message_object(msg_type='NewOrderSingle',
    ...
    # ######REQUEST TO VERIFY EXECUTION REPORT ON ORDER2##############
    ...
    order3_response = sf.placeOrderFIX(
        act=factory['act'],
        place_message_request=PlaceMessageRequest(
            description=f'Trader "{input_parameters["trader2"]}" sends request to create IOC Order.',
    ...
    # ######REQUEST TO VERIFY EXECUTION REPORTS ON TRADE##############
    ...
    ver2_chain = sf.submitCheckSequenceRule(
        check=factory['check'],
        check_sequence_rule_request=CheckSequenceRuleRequest(
            description=f'Trader "{input_parameters["trader2"]}" receives Execution Reports: '
...
def request_security_status(instrument, session_alias, event_id, factory):
    ...
    sf.sendMessage(
        ...
        place_message_request=PlaceMessageRequest(
            ...
            message=sf.create_message_object(msg_type='SecurityStatusRequest',
...
def scenario():
    ...
    sf.submit_event(
        estore=factory['estore'],
        event_batch=sf.create_event_batch(
            report_name="Aggressive IOC vs two orders: second order's price is lower than first",
    ...
    sf.submit_event(
        estore=factory['estore'],
        event_batch=sf.create_event_batch(
            report_name="Request security statuses",
            start_timestamp=report_start_timestamp,
            event_id=mdata_requests_report,
            parent_id=report_id))

    for instrument in instruments:
        request_security_status(instrument['SecurityID'], trader1_fix, mdata_requests_report, factory)
    ##################################################################
    ...
    for instrument in instruments:
        ver1_chain, ver2_chain = aggressive_ioc_traded_against_two_orders_partially_and_then_cancelled(
            f"Case: Trader {trader1} vs trader {trader2} for instrument {instrument['SecurityID']}",
            report_id, {
